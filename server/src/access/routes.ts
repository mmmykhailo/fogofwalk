...
function applicant(
  request: Awaited<ReturnType<ServerStore["getAccessRequest"]>>
): AccessRequest | null {
  return request
...
export function createAccessRoutes(store: ServerStore) {
  ...
  app.get("/access-request", requireSession, async (c) =>
    c.json(applicant(await store.getAccessRequest(c.get("user").id)))
  )
