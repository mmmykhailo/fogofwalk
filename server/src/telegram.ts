...
async function key() {
  ...
  return crypto.subtle.deriveKey({ name: "HKDF", hash: "SHA-256", salt, info }, material, { name: "AES-GCM", length: 256 }, false, ["encrypt", "decrypt"])
}
...
export async function decryptTelegramToken(envelope: string): Promise<string | null> {
  try {
    const [version, rawIv, rawCiphertext] = envelope.split(".")
    if (version !== "v1" || !rawIv || !rawCiphertext) return null
    const plaintext = await crypto.subtle.decrypt({ name: "AES-GCM", iv: Buffer.from(rawIv, "base64") }, await key(), Buffer.from(rawCiphertext, "base64"))
    return decoder.decode(plaintext)
  } catch { return null }
}

export async function sendTelegram(store: ServerStore, text: string): Promise<"not_configured" | "sent" | "failed"> {
  const [chatId, savedToken] = await Promise.all([store.getSetting("telegram_chat_id"), store.getSetting("telegram_bot_token")])
  ...
  try {
    const response = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
      ...
      body: JSON.stringify({ chat_id: chatId, text }), signal: AbortSignal.timeout(5_000),
    })
