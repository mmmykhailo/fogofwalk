...
Import progress is a batch snapshot over the bounded parser pool: each selected file has its own lifecycle stage, while the UI keeps every reached non-terminal stage as a lifecycle-ordered row for the whole operation. Each determinate bar counts files settled for that stage out of the immutable number of selected files; a rejected, failed, or cancelled file settles every displayed stage it can no longer reach. `completedFiles` counts files whose local reading, parsing, normalization, validation, and hashing preparation has ended; an accepted file then waits at `Waiting to save` until the one ordered durable library commit starts. Reached rows and their final settled counts remain visible for two seconds after a complete, partial, failed, or cancelled outcome. Import commit progress and downstream fog-worker progress remain separate surfaces and can be visible at the same time.
