...
import { PMTiles, FileSource, bytesToHeader, readVarint, tileIdToZxy } from "pmtiles"
import { VectorTile } from "@mapbox/vector-tile"
import Pbf from "pbf"

const PALETTE = new Set([
  "#d9272e", "#15803d", "#1769aa", "#eab308",
  "#ea580c", "#7e22ce", "#262626", "#854d0e",
])
...
if (header.specVersion !== 3 || header.tileType !== 1) throw new Error("archive must be PMTiles v3 MVT")
if (header.minZoom !== 12 || header.maxZoom !== 12) throw new Error("archive must contain z12 tiles only")
if (bytes.length > 0 && header.tileDataOffset + header.tileDataLength > bytes.length) throw new Error("tile data exceeds archive")

const file = new File([bytes], archivePath.split("/").pop() ?? "trails.pmtiles")
const archive = new PMTiles(new FileSource(file))
const metadata = await archive.getMetadata()
if (!String(metadata.attribution ?? "").toLowerCase().includes("openstreetmap")) throw new Error("missing OpenStreetMap attribution")
const layers = metadata.vector_layers
if (!Array.isArray(layers) || layers.length !== 1 || layers[0]?.id !== "trails") throw new Error("source layer must be trails")
const fields = layers[0]?.fields
if (!fields || Object.keys(fields).sort().join(",") !== "color,kind,offset,sort") throw new Error("unexpected trail properties")
...
for (let index = 0; index < entryCount; index++) {
  ...
  const offset = encoded === 0 && index > 0 ? previousOffset + previousLength : encoded - 1
  offsets.push(offset)
...
for (let index = 0; index < ids.length; index++) {
  if (lengths[index] > 1_048_576) throw new Error(`tile ${ids[index]} exceeds 1 MiB compressed`) 
  const [zoom, x, y] = tileIdToZxy(ids[index])
  if (zoom !== 12 || runs[index] !== 1) throw new Error("unexpected tile directory entry")
  ...
  for (let featureIndex = 0; featureIndex < layer.length; featureIndex++) {
    ...
    if (feature.type !== 2) throw new Error("trail archive contains a non-line feature")
    const properties = feature.properties
    if (Object.keys(properties).some((key) => !ALLOWED_PROPERTIES.has(key)) || Object.keys(properties).length !== 4) {
      throw new Error("trail feature has unexpected properties")
    }
    if (properties.kind !== "hiking" && properties.kind !== "cycling") throw new Error("unknown trail kind")
    if (properties.kind === "hiking" && !PALETTE.has(properties.color)) throw new Error("invalid hiking color")
    if (properties.kind === "cycling" && properties.color !== "#ec4899") throw new Error("invalid cycling color")
    if (typeof properties.offset !== "number" || !Number.isFinite(properties.offset)) throw new Error("invalid trail offset")
    if (typeof properties.sort !== "number" || !Number.isInteger(properties.sort) || properties.sort < (properties.kind === "hiking" ? 10 : 20) || properties.sort > (properties.kind === "hiking" ? 14 : 24)) throw new Error("invalid trail sort")
    featureCount++
  }
}
...
if (expectedChecksum) {
  ...
  if (actual !== expected.toLowerCase()) throw new Error("archive checksum mismatch")
}
...
console.log(JSON.stringify({ archive: archivePath, tiles: entryCount, features: featureCount }))
