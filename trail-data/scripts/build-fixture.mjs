...
const DEFAULT_OUTPUT = new URL("../../e2e/fixtures/trails-v1.pmtiles", import.meta.url)
const DEFAULT_EXPECTED = new URL("../fixtures/expected-z12.json", import.meta.url)
...
const COLOR_ORDER = ["red", "green", "blue", "yellow", "orange", "purple", "black", "brown"]
...
function members(source) {
  return [...source.matchAll(/<member\s+([^>]*?)\/?>(?:\s*<\/member>)?/g)].map((match) => {
    const value = attrs(match[1])
    return { type: value.type, ref: Number(value.ref), role: value.role ?? "" }
  })
}
...
function parseOsm(xml) {
  ...
  for (const match of xml.matchAll(/<way\s+([^>]*?)>([\s\S]*?)<\/way>/g)) {
    const value = attrs(match[1])
    const refs = [...match[2].matchAll(/<nd\s+([^>]*?)\/?>(?:\s*<\/nd>)?/g)]
      .map((nd) => Number(attrs(nd[1]).ref))
    ways.set(Number(value.id), { nodes: refs })
  }

  const relations = new Map()
  for (const match of xml.matchAll(/<relation\s+([^>]*?)>([\s\S]*?)<\/relation>/g)) {
    const value = attrs(match[1])
...
function normalizeColor(relationTags) {
  ...
  return { color: COLORS[token] ?? COLORS.purple, unsupported: COLORS[token] === undefined }
}

function classify(id, relationTags) {
  if (relationTags.type !== "route" && relationTags.type !== "superroute") return null
  const kind = relationTags.route === "bicycle"
    ? "cycling"
    : relationTags.route === "hiking" || relationTags.route === "foot"
      ? "hiking"
      : null
  if (!kind) return null
  const color = kind === "cycling" ? "#ec4899" : normalizeColor(relationTags).color
  const rank = (kind === "hiking" ? HIKING_RANKS : CYCLING_RANKS)[relationTags.network] ?? 0
  return { id, kind, rank, color, superroute: relationTags.type === "superroute" }
}

function visualFeatures(memberships) {
  const byKey = new Map()
  for (const membership of memberships) {
    const key = membership.kind === "hiking" ? `${membership.kind}:${membership.color}` : membership.kind
    const previous = byKey.get(key)
    if (!previous || membership.rank > previous.rank || (membership.rank === previous.rank && membership.id < previous.id)) {
      byKey.set(key, membership)
    }
  }
  const hiking = [...byKey.values()]
    .filter((membership) => membership.kind === "hiking")
    .sort((a, b) => b.rank - a.rank || COLOR_ORDER.indexOf(a.colorName) - COLOR_ORDER.indexOf(b.colorName) || a.id - b.id)
  const cycling = [...byKey.values()].find((membership) => membership.kind === "cycling")
  ...
  if (cycling) result.push({ ...cycling, offset: 0, sort: 20 + cycling.rank, color: "#ec4899" })
...
function selectFeatures(osm) {
  ...
  for (const [id, classification] of accepted) walkRelation(id, classification, new Set())
  ...
  for (const [wayId, memberships] of membershipsByWay) {
    ...
    const coordinates = way.nodes.map((nodeId) => osm.nodes.get(nodeId)).filter(Boolean)
    if (coordinates.length !== way.nodes.length || coordinates.length < 2) continue
    const normalized = memberships.map((membership) => ({
      id: membership.id,
      kind: membership.kind,
      rank: membership.rank,
      color: membership.color,
      colorName: COLOR_ORDER.find((name) => COLORS[name] === membership.color) ?? "purple",
    }))
...
function encodeDirectory(entries) {
  ...
  entries.forEach((entry, index) => {
    const encodedOffset = index > 0 && entry.offset === previousOffset + previousLength
      ? 0
      : entry.offset + 1
...
function writeHeader({ root, metadata, tileData, entries, bounds, center }) {
  ...
  uint64(view, 72, entries.reduce((sum, entry) => sum + entry.runLength, 0))
...
async function build(inputPath, outputPath, expectedPath) {
  ...
  for (const feature of selected.features) {
    for (const coordinate of feature.geometry.coordinates) {
      ...
      for (let dx = -1; dx <= 1; dx++) {
        for (let dy = -1; dy <= 1; dy++) candidateTiles.add(`${tile.x + dx},${tile.y + dy}`)
      }
    }
  }
  ...
  for (const key of candidateTiles) {
    ...
    tileRecords.push({ x, y, tileId: zxyToTileId(ZOOM, x, y), bytes: compressed })
  }
  ...
  for (const record of tileRecords) {
    entries.push({ tileId: record.tileId, offset: tileOffset, length: record.bytes.length, runLength: 1 })
  ...
  const metadata = gzipSync(Buffer.from(JSON.stringify({
    ...
    description: "Synthetic OSM route-relation fixture for Fog of Walk trail schema v1",
    vector_layers: [{ id: "trails", description: "OSM route relation members", minzoom: ZOOM, maxzoom: ZOOM, fields: { kind: "String", color: "String", offset: "Number", sort: "Number" } }],
  })))
  const bounds = selected.features.flatMap((feature) => feature.geometry.coordinates).reduce((result, [lon, lat]) => ({
  ...
  const header = writeHeader({ root, metadata, tileData: concat(tileChunks), entries, bounds, center })
  const archive = concat([header, root, metadata, concat(tileChunks)])
  await mkdir(dirname(outputPath), { recursive: true })
  await writeFile(outputPath, archive)
  await writeFile(expectedPath, await readFile(new URL("../fixtures/expected-z12.json", import.meta.url)))
  console.log(`built ${outputPath} (${archive.length} bytes, ${tileRecords.length} tiles, ${selected.features.length} features)`)
}
...
const expectedPath = resolve(args.get("expected") ?? fileURLToPath(DEFAULT_EXPECTED))
